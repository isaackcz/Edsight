...
class FormManagementSystem {
    ...
    async loadCurrentUser() {
        try {
            ...
            if (adminRoleElement) {
                adminRoleElement.textContent = data.admin_level_display;
            }
        } catch (error) {
    ...
    async loadStatistics() {
        try {
            ...
            if (pendingCountElement) pendingCountElement.textContent = data.pending_review || 0;
            
            const approvedCountElement = document.getElementById('approved-count');
            if (approvedCountElement) approvedCountElement.textContent = data.approved_today || 0;
            
            const returnedCountElement = document.getElementById('returned-count');
            if (returnedCountElement) returnedCountElement.textContent = data.returned_for_revision || 0;
            
            const totalCountElement = document.getElementById('total-count');
            if (totalCountElement) totalCountElement.textContent = data.total_forms || 0;
            
            // Update trends (simplified for now)
            const pendingTrendElement = document.getElementById('pending-trend');
            if (pendingTrendElement) pendingTrendElement.innerHTML = `<i class="fas fa-arrow-up"></i> ${data.pending_trend || 'No change'}`;
            
            const approvedTrendElement = document.getElementById('approved-trend');
            if (approvedTrendElement) approvedTrendElement.innerHTML = `<i class="fas fa-arrow-up"></i> ${data.approved_trend || 'No change'}`;
            
            const returnedTrendElement = document.getElementById('returned-trend');
            if (returnedTrendElement) returnedTrendElement.innerHTML = `<i class="fas fa-arrow-down"></i> ${data.returned_trend || 'No change'}`;
            
            const totalTrendElement = document.getElementById('total-trend');
            if (totalTrendElement) totalTrendElement.innerHTML = `<i class="fas fa-arrow-up"></i> ${data.total_trend || 'No change'}`;
        } catch (error) {
    ...
    setupEventListeners() {
        ...
        const formDetailModal = document.getElementById('form-detail-modal');
        if (formDetailModal) {
            formDetailModal.addEventListener('click', (e) => {
