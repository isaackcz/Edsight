...
// No-op PreloaderManager stub (preloader removed)
...
// Initialize preloader manager stub
const preloaderManager = new PreloaderManager();
window.preloaderManager = preloaderManager;

// DOMContentLoaded initialization (no preloader actions)
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/api/profile/")
    ...
    .catch((error) => {
      // Authentication check failed; still try to load data
      loadUserData();
      loadDashboardData();
      loadAnalyticsData();
    });
...
function setupBreadcrumbNavigation() {
  ...
  breadcrumbItems.forEach((item) => {
    item.addEventListener("click", function (e) {
      ...
      const targetPage =
        this.getAttribute("data-page") || this.textContent.toLowerCase().trim();
        ...
        if (question.sub_questions && Array.isArray(question.sub_questions)) {
          question.sub_questions.forEach((subQuestion) => {
            if (
              ...
              subQuestion.sub_question_text.toLowerCase().includes(query)
            ) {
              results.push({
                type: "sub-question",
                id: subQuestion.sub_question_id,
                title: subQuestion.sub_question_text,
                subtitle: `Sub-question in ${question.question_text.substring(
                  0,
                  50
                )}...`,
...
function navigateToCategory(categoryId) {
  ...
  if (
    window.dashboardUIManager &&
    typeof window.dashboardUIManager.showCategoryDetails === "function"
  ) {
...
function navigateToSubSection(category, subSectionName) {
  ...
  if (
    ...
    setTimeout(() => {
      if (
        typeof window.dashboardUIManager.showSubSectionDetails === "function"
      ) {
...
function setupUserMenu() {
  ...
  const mobileUserAvatar = document.querySelector(
    ".mobile-header-actions .user-avatar"
  );

  // Desktop user avatar in header
  const headerUserAvatar = document.querySelector(
...
function updateUserInfo(userData) {
  ...
  const schoolName =
    userData.personal_info?.school_name ||
  ...
  if (profileLastLogin) {
    const lastLoginDate =
      userData.last_login ||
      userData.personal_info?.last_login ||
      userData.account_settings?.last_login;
    if (lastLoginDate) {
  ...
  if (userData.personal_info) {
    ...
    if (schoolNameInput)
      schoolNameInput.value = userData.personal_info.school_name || "";
    ...
    if (divisionInput)
      divisionInput.value = userData.personal_info.division || "";
    if (districtInput)
      districtInput.value = userData.personal_info.district || "";
  }
}
...
function loadDashboardData() {
  ...
  const apiCalls = [
    ...
    fetch("/api/dashboard/completion/")
      .then((res) => {
    ...
    fetch("/api/dashboard/recent-activity/")
      .then((res) => {
        if (!res.ok)
          throw new Error(`Recent Activity API failed: ${res.status}`);
        return res.json();
      })
      .catch((err) => {
...
function updateDashboardStats(data) {
  ...
  if (deadlinePercentage) {
    ...
    deadlinePercentage.textContent = `${Math.round(
      percentageUsed
    )}% of time used`;
  }
}
...
function updateChartsWithRealData(stats, completion) {
  ...
  if (window.categoryChart && completion && completion.categories) {
    const totalCompleted = completion.categories.reduce(
      (sum, cat) => sum + (cat.completed || 0),
      0
    );
    const totalInProgress = completion.categories.reduce(
      (sum, cat) => sum + (cat.in_progress || 0),
      0
    );
    const totalNotStarted = completion.categories.reduce(
      (sum, cat) => sum + (cat.not_started || 0),
      0
    );
...
function updateCategoryProgress(categories) {
  ...
  if (categoryChart && categories && categories.length > 0) {
    ...
    categories.forEach((cat) => {
      ...
      totalInProgress += Math.max(
        0,
        (cat.total_questions || 0) - (cat.completed_questions || 0) - 2
      );
...
function updateMainProgressChart(progress) {
  ...
  if (window.mainProgressChart) {
    ...
    window.mainProgressChart.data.datasets[0].data = [
      chartProgress,
      remainingProgress,
    ];
...
function loadSampleData() {
  ...
  const sampleCategories = [
    ...
    {
      category_name: "Academic Details",
...
function renderForm(data) {
  ...
  if (Array.isArray(data)) {
    ...
    formContainer.innerHTML =
      '<div class="error-message"><p>Invalid form data structure. Please check the API response.</p></div>';
    return;
  }
  ...
  if (categories.length > 0) {
    ...
    formContainer.innerHTML =
      '<div class="no-data-message"><p>No form sections available.</p></div>';
  }
...
function renderSubSection(subSection) {
  let subSectionHtml = `
        <div class="subsection-card" data-subsection="${
...
function generateCategoryCard(category) {
  ...
  const subsectionItems = subsections
    .slice(0, 3)
    .map((sub) => {
  ...
  return `
        <div class="category-card ${statusClass}" data-category-id="${
    category.name
  }">
            <div class="card-header">
                <div class="category-title-section">
                    <h4 class="category-name">${category.name}</h4>
                    <div class="completion-percentage">${Math.round(
                      completion
                    )}%</div>
                </div>
            </div>
...
function initializeCategoryInteractions() {
  ...
  document
    .querySelectorAll(".btn-view-details, .btn-view-details-inline")
    .forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (
          window.dashboardUIManager &&
          window.dashboardUIManager.formSections &&
          window.dashboardUIManager.formSections.length > 0
        ) {
...
function updateFormStatusHierarchy(categories) {
  ...
  if (categories && categories.length > 0) {
    ...
    const categoryCards = categories
      .map((category) => generateCategoryCard(category))
      .join("");
                ...
                <div class="tree-item">
                    ...
                    <div class="tree-percentage">
                        <div class="percentage-bar">
                            <div class="percentage-fill ${percentageClass}" style="width: ${
        item.percentage
      }%"></div>
                        </div>
                ...
                ${
                  hasChildren
                    ? `<div class="tree-children">${generateHierarchyHTML(
                        item.children
                      )}</div>`
                    : ""
                }
...
function loadActiveSessions() {
  ...
  sessionsList.innerHTML =
    '<div class="session-loading">Loading active sessions...</div>';
...
function renderActiveSessions(sessions) {
  ...
  if (sessions.length === 0) {
    sessionsList.innerHTML =
      '<div class="session-loading">No active sessions found.</div>';
    return;
  }
...
function loadLoginHistory() {
  ...
  historyList.innerHTML =
    '<div class="history-loading">Loading login history...</div>';
...
function renderLoginHistory(history) {
  ...
  if (history.length === 0) {
    historyList.innerHTML =
      '<div class="history-loading">No login history found.</div>';
    return;
  }
        ...
        <div class="history-item">
            <div class="history-info">
                <div class="history-device">
                    <i class="fas fa-${getDeviceIcon(item.device)} ${
...
function renderAuditLogs(logs) {
  ...
  if (logs.length === 0) {
    auditLogs.innerHTML =
      '<div class="audit-loading">No audit logs found.</div>';
    return;
  }
    ...
    .map((log) => {
      ...
      if (log.old_value) {
        if (typeof log.old_value === "object") {
          oldValueText =
            log.old_value.response || JSON.stringify(log.old_value);
        } else {
      ...
      if (log.new_value) {
        if (typeof log.new_value === "object") {
          newValueText =
            log.new_value.response || JSON.stringify(log.new_value);
        } else {
            ...
            <div class="audit-item ${log.resource_type}">
                <div class="audit-content">
                    ...
                    <div class="audit-status">
                        <span class="badge ${
                          log.saved_locally ? "badge-success" : "badge-warning"
                        }">
                            <i class="fas fa-${
                              log.saved_locally ? "check" : "clock"
                            }"></i>
                            Local: ${log.saved_locally ? "Saved" : "Pending"}
                        </span>
                        <span class="badge ${
                          log.saved_database ? "badge-success" : "badge-warning"
                        }">
                            <i class="fas fa-${
                              log.saved_database ? "check" : "clock"
                            }"></i>
...
async function loadSecuritySessions() {
  try {
    const response = await fetch(
      `/api/security/sessions?user_id=${currentUserId}`
    );
