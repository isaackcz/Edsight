...
function initCharts() {
    ...
    distributionChart = new Chart(distributionCtx, {
        ...
        data: { labels: [], datasets: [{
            data: [],
            backgroundColor: ['#293440', '#46576c', '#788b95', '#abc7d6', '#e0ebf6'],
            borderWidth: 0,
            hoverOffset: 10
        }]},
...
function setupEventListeners() {
    ...
    notification.addEventListener('click', function() {
        ...
        this.querySelector('.badge').style.backgroundColor = '#4caf50';
    });
...
async function fetchDashboardData() {
    try {
        ...
        const [stats, completion, timeline, schools, distribution] = await Promise.all([
            fetch(`${API_BASE}/api/dashboard/stats/`).then(res => res.json()),
            fetch(`${API_BASE}/api/dashboard/completion_by_region/`).then(res => res.json()),
            fetch(`${API_BASE}/api/dashboard/forms_over_time/`).then(res => res.json()),
            fetch(`${API_BASE}/api/dashboard/top_schools/`).then(res => res.json()),
            fetch(`${API_BASE}/api/dashboard/response_distribution/?question=1`).then(res => res.json())
        ]);
...
function updateStatsCards(data) {
    document.getElementById('total-forms').textContent = data.total_forms.toLocaleString();
    document.getElementById('total-forms-trend').innerHTML = 
        `<i class="fas fa-arrow-${data.total_forms_trend.direction}"></i> ${data.total_forms_trend.value} from last month`;
    
    document.getElementById('completion-rate').textContent = `${data.completion_rate}%`;
    document.getElementById('completion-rate-trend').innerHTML = 
        `<i class="fas fa-arrow-${data.completion_rate_trend.direction}"></i> ${data.completion_rate_trend.value} from last month`;
    
    document.getElementById('avg-time').textContent = `${data.avg_time} min`;
    document.getElementById('avg-time-trend').innerHTML = 
        `<i class="fas fa-arrow-${data.avg_time_trend.direction}"></i> ${data.avg_time_trend.value} from last month`;
    
    document.getElementById('active-schools').textContent = data.active_schools;
    document.getElementById('active-schools-trend').innerHTML = 
        `<i class="fas fa-arrow-${data.active_schools_trend.direction}"></i> ${data.active_schools_trend.value} from last month`;
...
function updateSchoolsTable(schools) {
    ...
    schools.forEach(school => {
        ...
        row.innerHTML = `
            ...
            <td><span class="badge ${school.status === 'Completed' ? 'success' : 'warning'}">${school.status}</span></td>
        `;
...
async function fetchResponseDistribution(questionId) {
    try {
        const API_BASE = 'http://localhost:8000';
        const response = await fetch(`${API_BASE}/api/dashboard/response_distribution/?question=${questionId}`);
