// Deadline Compliance Page JavaScript
let currentPage = 1;
let pageSize = 100;
let totalCount = 0;
...
let currentTableColumns = ['Area', 'Form Type', 'Deadline', 'Total Forms', 'On Time', 'Late', 'Overdue', 'Compliance Rate (%)'];
...
async function loadDeadlinesData() {
    try {
        ...
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center">Loading deadline data...</td></tr>';
        }
        ...
        const response = await fetch('/api/reports/deadline-compliance/', {
            method: 'POST',
            headers: { 
        ...
        if (data.success && data.data) {
            ...
            renderDeadlinesTable(data.data);
            updatePagination();
        } else if (data.error) {
...
function initializeSearch() {
    const searchInput = document.getElementById('deadlines-search');
...
function renderDeadlinesTable(data) {
    ...
    if (data.deadlines.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center">No deadline data available for the selected filters</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = data.deadlines.map(deadline => {
        ...
        return `
            <tr>
                <td>${deadline.area_name || 'Unknown'}</td>
                <td>${deadline.form_type || 'Unknown'}</td>
                <td class="${statusClass}">${formattedDate} ${daysUntil < 0 ? '(Overdue)' : daysUntil <= 7 ? `(${daysUntil} days)` : ''}</td>
                <td>${(deadline.total_forms || 0).toLocaleString()}</td>
                <td class="text-success">${(deadline.on_time || 0).toLocaleString()}</td>
                <td class="text-warning">${(deadline.late || 0).toLocaleString()}</td>
                <td class="text-danger">${(deadline.overdue || 0).toLocaleString()}</td>
                <td>${(deadline.compliance_rate || 0).toFixed(2)}%</td>
            </tr>
        `;
    }).join('');
    
    addTableSorting('deadlines-table');
}
...
function handleExport() {
    ...
    const isDefaultFilters = dateRange === 'all' && !dateFrom && !dateTo && level === 'all' && !currentSearch;
    
    if (isDefaultFilters) {
        // Export all data directly
        exportDeadlinesTableData();
    } else {
...
function exportDeadlinesTableData() {
    ...
    const dateFrom = filters.date_from || '';
    const dateTo = filters.date_to || '';
    let filename = 'deadline_compliance';
    if (dateFrom && dateTo) {
