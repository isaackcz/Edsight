...
def logs_page(request):
    ...
    educational_logs = AuditLog.objects.filter(
        timestamp__range=(start_dt, end_dt),
        resource_type__in=['question', 'answer', 'form', 'category', 'topic', 'sub_section']
    ).select_related('user').order_by('-timestamp')
