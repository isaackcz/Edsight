...
from .models import (
    Form, Answer, Question, SubQuestion, AdminUser, 
    Region, Division, District, Category, SubSection, Topic
)
...
class AnalyticsService:
        ...
        if level == 'category':
            ...
            for category in categories:
                ...
                category_questions = Question.objects.filter(
                    topic__sub_section__category=category
                )
            ...
            # Drill down by subsection
            subsections = SubSection.objects.all().order_by('display_order')
            
            for subsection in subsections:
                subsection_questions = Question.objects.filter(
                    topic__sub_section=subsection
                )
                
                subsection_answers = Answer.objects.filter(
                    question__in=subsection_questions,
                    form__in=queryset,
                    response__isnull=False
                ).exclude(response='')
                
                count = subsection_answers.count()
                total_possible = subsection_questions.count() * queryset.count()
                percentage = (count / total_possible * 100) if total_possible > 0 else 0
    ...
    def get_filter_options():
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # Get subsections with category relationships
                cursor.execute("SELECT sub_section_id, name, category_id FROM sub_sections ORDER BY display_order")
                sub_sections = [{'sub_section_id': row[0], 'name': row[1], 'category_id': row[2]} for row in cursor.fetchall()]
                
                # Get topics with subsection relationships
                cursor.execute("SELECT topic_id, name, sub_section_id FROM topics ORDER BY display_order")
                topics = [{'topic_id': row[0], 'name': row[1], 'sub_section_id': row[2]} for row in cursor.fetchall()]
                ...
                # Get sub-questions with question relationships
                cursor.execute("""
                    SELECT 
                        sq.sub_question_id, 
                    ...
                    LEFT JOIN answers a ON sq.sub_question_id = a.sub_question_id
                    GROUP BY sq.sub_question_id, sq.sub_question_text, sq.question_id
                    ORDER BY sq.display_order
                """)
                sub_questions = [{'sub_question_id': row[0], 'sub_question_text': row[1], 'question_id': row[2], 
                               'answer_count': row[3]} for row in cursor.fetchall()]
    ...
    def get_category_content_data(queryset, filters):
        ...
        with connection.cursor() as cursor:
            ...
            LEFT JOIN sub_questions sq ON a.sub_question_id = sq.sub_question_id
            LEFT JOIN topics t ON q.topic_id = t.topic_id
            LEFT JOIN sub_sections ss ON t.sub_section_id = ss.sub_section_id
            LEFT JOIN categories c ON ss.category_id = c.category_id
            WHERE us.is_active = 1 
            AND EXISTS (
            ...
            for row in rows:
                (school_id, school_name, region_id, region_name, division_id, division_name, 
                 district_id, district_name, category_id, category_name, subsection_id, 
                 subsection_name, topic_id, topic_name, question_id, question_text, 
                 answer_type, is_required, sub_question_id, sub_question_text, 
                 answer_id, response, form_status) = row
                
                # Only include rows where we have actual content structure and the school has forms
                if category_id and subsection_id and topic_id and question_id:
                    content_data.append({
                        'school_id': school_id,
                        'school_name': school_name or 'Unknown School',
                        'region_id': region_id,
                        ...
                        'subsection': subsection_name or 'Unknown Subsection',
                        'topic_id': topic_id,
                        'topic': topic_name or 'Unknown Topic',
                        'question_id': question_id,
                        'question': question_text or 'Unknown Question',
                        'sub_question': sub_question_text or '',
    ...
    def get_hierarchical_filter_options(filter_type, parent_id):
        ...
        try:
            with connection.cursor() as cursor:
                if filter_type == 'division' and parent_id:
                ...
                elif filter_type == 'subsection' and parent_id:
                    # Get subsections for a specific category
                    cursor.execute("""
                        SELECT sub_section_id, name 
                        FROM sub_sections 
                        WHERE category_id = %s 
                        ORDER BY display_order
                    """, [parent_id])
                    return [{'sub_section_id': row[0], 'name': row[1]} for row in cursor.fetchall()]
                
                elif filter_type == 'topic' and parent_id:
                    # Get topics for a specific subsection
                    cursor.execute("""
                    ...
                    cursor.execute("""
                        SELECT sub_question_id, sub_question_text, answer_type
                        FROM sub_questions 
                        WHERE question_id = %s 
                        ORDER BY display_order
                    """, [parent_id])
                    return [{'sub_question_id': row[0], 'sub_question_text': row[1], 'answer_type': row[2]} for row in cursor.fetchall()]
