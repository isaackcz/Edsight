// Admin Activity Page JavaScript
let currentPage = 1;
let pageSize = 100;
let totalCount = 0;
let searchTimeout = null;
let currentSearch = '';
let currentTableColumns = ['Username', 'Full Name', 'Admin Level', 'Activity Count', 'Login Count', 'Active Sessions', 'Avg. Session (hours)', 'Last Activity'];
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        if (tbody && searchValue !== currentSearch) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center">Searching...</td></tr>';
        }
    });
}
...
async function loadAdminActivityData() {
    try {
        const tbody = document.getElementById('admin-activity-tbody');
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center">Loading admin data...</td></tr>';
        }
        ...
        const response = await fetch('/api/reports/admin-activity/', {
            method: 'POST',
            headers: { 
        ...
        if (data.success && data.data) {
            ...
            renderAdminActivityTable(data.data);
            updatePagination();
        } else if (data.error) {
...
function renderAdminActivityTable(data) {
    const tbody = document.getElementById('admin-activity-tbody');
    if (!tbody || !data.admins) return;
    
    if (data.admins.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center">No admin data available for the selected filters</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = data.admins.map(admin => {
        ...
        return `
            <tr class="${activityClass}">
                ...
                <td>${admin.full_name || admin.username || 'Unknown'}</td>
                <td>${admin.admin_level || 'N/A'}</td>
                <td>${(admin.activity_count || 0).toLocaleString()}</td>
                <td>${(admin.login_count || 0).toLocaleString()}</td>
                <td>${admin.active_sessions || 0}</td>
                <td>${(admin.avg_session_duration_hours || 0).toFixed(2)}</td>
                <td>${lastActivity}</td>
            </tr>
        `;
    }).join('');
...
function handleExport() {
    ...
    const adminLevel = document.getElementById('admin-level-filter')?.value || '';
    
    // Check if filters are default
    const isDefaultFilters = dateRange === 'all' && !dateFrom && !dateTo && !adminLevel && !currentSearch;
    
    if (isDefaultFilters) {
        // Export all data directly
        exportAdminActivityTableData();
    } else {
        // Show export modal with filter info
        showExportModal(filters, adminLevel);
    }
}

function showExportModal(filters, adminLevel) {
    ...
    if (adminLevel) {
        filterText += `• Level: ${adminLevel.charAt(0).toUpperCase() + adminLevel.slice(1)}\n`;
    }
    if (currentSearch) {
...
function exportAdminActivityTableData() {
    ...
    const adminLevel = document.getElementById('admin-level-filter')?.value || '';
    if (adminLevel) filename += `_level_${adminLevel}`;
    if (currentSearch) {
